...
FireObject::FireObject(const std::string& name, const std::string& color_buf_name, const std::string& depth_buf_name)
...
void FireObject::init(Configuration& cfg, RenderAttachments& attachments)
{
...
void FireObject::createFramebuffer()
{
...
void FireObject::createRenderPass()
{
...
void FireObject::createPipeline(Configuration& cfg)
{
    {
...

    {
        ...
        auto vertShaderCode = readFile(cfg.at("shader_directory").get<std::string>() + "/fire_object/node.vert.spv");
        auto fragShaderCode = readFile(cfg.at("shader_directory").get<std::string>() + "/fire_object/node.frag.spv");
    ...
    {
        assert(g_ctx.rm->fields.has_temperature);
        ...
        pipeline.param.fire_lights = g_ctx.dm.getResourceHandle(g_ctx.rm->fields.lights.buffer.id);
...
void FireObject::record(uint32_t swapchain_index)
{
