...
void FireFieldNode::init(Configuration& cfg, RenderAttachments& attachments)
{
...
void FireFieldNode::createFramebuffer()
{
...
void FireFieldNode::createRenderPass()
{
...
void FireFieldNode::createPipeline(Configuration& cfg)
{
    {
...

    {
        ...
        auto vertShaderCode = readFile(cfg.shader_directory + "/fire_field/node.vert.spv");

        auto frag_shader_path = std::filesystem::path(cfg.engine_directory) / "function/render/render_graph/node/fire_field/node.frag";
        auto filename = frag_shader_path.filename().string();
        auto generated_path = cfg.shader_directory + "/fire_field/generated/" + filename;
        auto generated_spv = cfg.shader_directory + "/fire_field/" + (filename + ".spv");
...
void FireFieldNode::record(uint32_t swapchain_index)
{
